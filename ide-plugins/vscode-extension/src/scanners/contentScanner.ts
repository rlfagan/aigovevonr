...
export class ContentScanner {
    ...
    // Proprietary markers
    private readonly PROPRIETARY_MARKERS = [
        '@proprietary',
        '@confidential',
        '@internal',
    ...
    public scanContent(content: string): ScanResult {
    ...
    private isMostlySafeContent(content: string): boolean {
    ...
    public scanForCredentials(content: string): boolean {
        for (const secret of this.SECRET_PATTERNS) {
    ...
    public scanForPII(content: string): boolean {
        for (const pii of this.PII_PATTERNS) {
            const matches = content.match(pii.pattern);
            if (matches) {
    ...
    public scanForProprietaryMarkers(content: string): boolean {
        return this.PROPRIETARY_MARKERS.some(marker =>
